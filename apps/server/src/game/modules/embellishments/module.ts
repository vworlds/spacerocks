...
function isAlive(world: World, entity: Entity | undefined): entity is Entity {
  return entity !== undefined && world.getEntity(entity.eid) !== undefined;
}

function createChild(world: World, parent: Entity): Entity {
  return world
    .entity()
    .add(Networked)
...
/**
 * Visual embellishments: health bars, shield rings, and laser beams attached
    ...
    world
      ...
      .each(
        ...
        (entity, [rotation, parentRotation]) => {
          if (!parentRotation || rotation.angle === parentRotation.angle)
            return;
